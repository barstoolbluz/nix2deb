    ...
    in
    {
      ...
      checks = forAllSystems (system:
        ...
        in
        {
          ...
          in pkgs.runCommand "test-leftover-refs-fails" {} ''
          ...
          # Test 8: Unresolved DT_NEEDED should fail the build
          unresolved-dep-fails = let
            badDeb = nixToDeb {
              inherit pkgs;
              package = fixtures.simpleElf;
              ...
              version = "0.1.0";
              binName = "simple-elf";
              excludeLibs = [ "glibc" "zlib" ];
              description = "Should fail on unresolved dep";
            };
            failed = pkgs.testers.testBuildFailure badDeb;
          in pkgs.runCommand "test-unresolved-dep-fails" {} ''
            exitCode=$(cat ${failed}/testBuildFailure.exit)
            if [ "$exitCode" -eq 0 ]; then
              echo "FAIL: build should have failed but exited 0"
              exit 1
          ...
          # Test 7: Real-world package (hello)
